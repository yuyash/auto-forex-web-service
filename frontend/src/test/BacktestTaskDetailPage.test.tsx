...
function renderWithProviders(ui: React.ReactElement, taskId = '1') {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <AuthProvider>
        <MemoryRouter initialEntries={[`/backtest-tasks/${taskId}`]}>
          <Routes>
            <Route path="/backtest-tasks/:id" element={ui} />
          </Routes>
        </MemoryRouter>
      </AuthProvider>
    </QueryClientProvider>
...
describe('BacktestTaskDetailPage Integration Tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    MockWebSocket.reset();
    ...
    mockFetch.mockImplementation((url: string) => {
      ...
      if (url.includes('/api/backtest-tasks/1/')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve(mockBacktestTask),
        });
      }
  ...
  it('should connect to WebSocket on mount (Requirement 3.3)', async () => {
    renderWithProviders(<BacktestTaskDetailPage />);
  ...
  it('should fetch latest status from backend before rendering (Requirement 3.6)', async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/backtest-tasks/1/'),
        expect.any(Object)
      );
    });
  });
  ...
  it('should display log panel with real-time log streaming (Requirement 6.8)', async () => {
    ...
    await waitFor(() => {
      const logWsInstance = MockWebSocket.instances.find((ws) =>
        ws.url.includes('/ws/tasks/backtest/1/logs/')
      );
  ...
  it('should show correct action buttons based on task status (Requirement 4.5)', async () => {
    ...
    // For running task, should show Stop button
  ...
  it('should handle button interactions with optimistic updates (Requirement 3.1)', async () => {
    ...
    mockFetch.mockImplementation((url: string) => {
      ...
      if (url.includes('/api/backtest-tasks/1/stop/')) {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({
