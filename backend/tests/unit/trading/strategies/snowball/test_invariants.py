...
class TestSnowballInvariantValidator:
    """Validate Snowball structural invariant reporting."""

    def test_accepts_consistent_grid_state(self):
        ...
        slot.entry = factory.entry()
        state = SnowballStrategyState(
            cycles=[
                SnowballCycle(
                    cycle_id=1,
                    direction=Direction.LONG,
                    grid=factory.layer_grid(layer),
                )
            ],
            next_entry_id=2,
        )
    ...
    def test_reports_duplicate_entry_id(self):
        factory = SnowballInvariantTestFactory()
        ...
        state = SnowballStrategyState(
            cycles=[
                SnowballCycle(
                    cycle_id=1,
                    direction=Direction.LONG,
                    grid=factory.layer_grid(first),
                ),
                SnowballCycle(
                    cycle_id=2,
                    direction=Direction.LONG,
                    grid=factory.layer_grid(second),
                ),
            ],
            next_entry_id=2,
        )
