...
from trading.services.task_lock_manager import HeartbeatData, LockInfo, TaskLockManager
...
class TestLockAcquisition:
    ...
    def test_acquire_lock_success(self, lock_manager):
        ...
        lock_key = lock_manager._get_lock_key(TaskType.BACKTEST, 1)
    ...
    def test_acquire_lock_failure_already_held(self, lock_manager):
        ...
        result1 = lock_manager.acquire_lock(
            task_type=TaskType.BACKTEST, task_id=1, execution_id=100
        )
        assert result1 is True

        # Second acquisition fails
        result2 = lock_manager.acquire_lock(
            task_type=TaskType.BACKTEST, task_id=1, execution_id=101
        )
    ...
    def test_acquire_lock_different_tasks(self, lock_manager):
        ...
        result2 = lock_manager.acquire_lock(
            task_type=TaskType.BACKTEST, task_id=2, execution_id=101
        )
    ...
    def test_acquire_lock_different_task_types(self, lock_manager):
        ...
        result2 = lock_manager.acquire_lock(task_type=TaskType.TRADING, task_id=1, execution_id=101)
        assert result2 is True

    def test_acquire_lock_sets_heartbeat(self, lock_manager):
        """Test that acquiring lock also sets initial heartbeat."""
        # Acquire lock
        lock_manager.acquire_lock(task_type=TaskType.BACKTEST, task_id=1, execution_id=100)

        # Verify heartbeat is set
        heartbeat_key = lock_manager._get_heartbeat_key(TaskType.BACKTEST, 1)
...
class TestLockRelease:
    ...
    def test_release_lock_success(self, lock_manager):
        ...
        # Release lock
        lock_manager.release_lock(task_type=TaskType.BACKTEST, task_id=1)
    ...
    def test_release_lock_removes_cancellation_flag(self, lock_manager):
        ...
        lock_manager.acquire_lock(task_type=TaskType.BACKTEST, task_id=1, execution_id=100)
        lock_manager.set_cancellation_flag(task_type=TaskType.BACKTEST, task_id=1)

        # Release lock
        lock_manager.release_lock(task_type=TaskType.BACKTEST, task_id=1)

        # Verify cancellation flag is removed
        cancel_key = lock_manager._get_cancellation_key(TaskType.BACKTEST, 1)
    ...
    def test_release_nonexistent_lock(self, lock_manager):
        ...
        lock_manager.release_lock(task_type=TaskType.BACKTEST, task_id=999)
    ...
    def test_reacquire_after_release(self, lock_manager):
        ...
        result = lock_manager.acquire_lock(task_type=TaskType.BACKTEST, task_id=1, execution_id=101)
        assert result is True
...
class TestHeartbeat:
    ...
    def test_update_heartbeat_success(self, lock_manager):
        ...
        time.sleep(0.1)
        lock_manager.update_heartbeat(task_type=TaskType.BACKTEST, task_id=1)
    ...
    def test_update_heartbeat_without_lock(self, lock_manager):
        ...
        # Should not raise any exception
        lock_manager.update_heartbeat(task_type=TaskType.BACKTEST, task_id=999)
...
class TestCancellationFlag:
    ...
    def test_check_cancellation_flag_true(self, lock_manager):
        ...
        # Check flag
        is_cancelled = lock_manager.check_cancellation_flag(task_type=TaskType.BACKTEST, task_id=1)
    ...
    def test_cancellation_flag_workflow(self, lock_manager):
        ...
        assert lock_manager.check_cancellation_flag(TaskType.BACKTEST, 1) is False

        # Set cancellation flag
        lock_manager.set_cancellation_flag(task_type=TaskType.BACKTEST, task_id=1)

        # Now should be cancelled
        assert lock_manager.check_cancellation_flag(TaskType.BACKTEST, 1) is True
...
class TestStaleLockCleanup:
    ...
    def test_cleanup_stale_lock_with_old_heartbeat(self, lock_manager):
        ...
        # Cleanup stale lock
        was_stale = lock_manager.cleanup_stale_lock(task_type=TaskType.BACKTEST, task_id=1)
    ...
    def test_cleanup_nonexistent_lock(self, lock_manager):
        ...
        was_stale = lock_manager.cleanup_stale_lock(task_type=TaskType.BACKTEST, task_id=999)
        assert was_stale is False
...
class TestLockInfo:
    ...
    def test_get_lock_info_with_active_lock(self, lock_manager):
        ...
        # Get lock info
        info = lock_manager.get_lock_info(task_type=TaskType.BACKTEST, task_id=1)
    ...
    def test_get_lock_info_nonexistent_lock(self, lock_manager):
        ...
        info = lock_manager.get_lock_info(task_type=TaskType.BACKTEST, task_id=999)
        assert info is None
...
class TestConcurrentAccess:
    ...
    def test_concurrent_acquisition_only_one_succeeds(self, lock_manager):
        ...
        for i in range(5):
            result = lock_manager.acquire_lock(
                task_type=TaskType.BACKTEST, task_id=1, execution_id=100 + i
            )
