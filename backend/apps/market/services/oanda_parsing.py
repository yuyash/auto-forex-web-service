...
def parse_order(order: Any) -> Order:
    raw_type = str(object_field(order, "type") or "").upper()
    order_type = {
    ...
    units_signed = to_decimal(object_field(order, "units")) or Decimal("0")
    direction = (
        OrderDirection.LONG
        if units_signed > 0
    ...
    price = to_decimal(object_field(order, "price"))

    create_time = parse_iso_datetime(object_field(order, "createTime"))
    fill_time = parse_iso_datetime(object_field(order, "filledTime"))
    cancel_time = parse_iso_datetime(object_field(order, "cancelledTime"))
    state = OrderState.from_raw(object_field(order, "state"))
    time_in_force_raw = object_field(order, "timeInForce")
    time_in_force = str(time_in_force_raw) if time_in_force_raw else None

    order_id = str(object_field(order, "id") or "")
    instrument = str(object_field(order, "instrument") or "")
...
def parse_transaction(txn: dict[str, Any]) -> Transaction:
    ...
    return Transaction(
        ...
        time=parse_iso_datetime(txn.get("time")),
        type=str(txn.get("type", "")),
        instrument=str(txn.get("instrument")) if txn.get("instrument") else None,
        units=to_decimal(txn.get("units")),
        price=to_decimal(txn.get("price")),
        pl=to_decimal(txn.get("pl")),
        account_balance=to_decimal(txn.get("accountBalance")),
