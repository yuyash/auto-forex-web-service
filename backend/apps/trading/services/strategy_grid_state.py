...
def build_cycle_grid_state_map(
    *,
    ...
    """Return cycle_id -> grid state mappings for supported strategies."""
    if strategy_type != "snowball" or not isinstance(strategy_state, dict):
        return {}
    ...
    for cycle in state.cycles:
        grid_state = _serialize_snowball_cycle_grid(cycle)
        cycle_keys = {
            str(key)
...
def _serialize_snowball_cycle_grid(cycle: Any) -> dict[str, Any]:
