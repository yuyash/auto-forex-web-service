...
ロットサイズは `counter_close_count`（カウンターTPクローズの累計回数）によって異なる:

- 初回サイクル（`counter_close_count == 0`）: $lot = (layer\_retracement\_count + 2) \times cycle\_base\_units$
- 段別決済後（`counter_close_count > 0`）: $lot = (layer\_retracement\_count + 1) \times cycle\_base\_units$

初回サイクルで +2 となるのは、初期エントリーを1番目のポジションとして数えるため。段別決済後は `layer_retracement_count` が0にリセットされるため、1ロットから再スタートする。
...
3. 次のtick以降で再度逆行した場合、ロット `1 × base_units` = 1,000 から再スタート
4. ロットは `(layer_retracement_count + 1) × base_units`（段別決済後）
...
5. 以降、レイヤー2として同じロジックでカウンター追加を再開
